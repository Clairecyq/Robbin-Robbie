...
public class CharacterController2D : MonoBehaviour
{
    ...
    public float currentHidingPower;
    ...
    public float getMaxHidingEnergy()
    {
    ...
    private void transformationUpdate(int fr) {
        // logging check
        float powerRatio = (currentHidingPower) / getMaxHidingEnergy();
    ...
    private void regen() {
        regen_counter += Time.deltaTime;
        if (regen_counter > lockout)
        {
            currentHidingPower = Mathf.Min(startingHidingPower, currentHidingPower + Time.deltaTime *regenpersec);
        }
